            ...
            if ([[NSFileManager defaultManager] fileExistsAtPath:[[paths objectAtIndex:0] stringByAppendingString:@"/EpiInfoForms"]] && formName.length > 0)
            {
                ...
                if ([[NSFileManager defaultManager] fileExistsAtPath:[[paths objectAtIndex:0] stringByAppendingString:@"/GoogleSheetDatabase"]])
                {
                    ...
                    if (tableAlreadyExists >= 1)
                    {
                        ...
                        {
                            char *errMsg;
                            NSString *insertStatement = [NSString stringWithFormat:@"delete from GoogleSheets where FormName = '%@'", formName];
                            const char *sql_stmt = [insertStatement UTF8String];
                            ...
                            insertStatement = [NSString stringWithFormat:@"insert into GoogleSheets values('%@', '%@')", formName, sheetURLString];
                            sql_stmt = [insertStatement UTF8String];
