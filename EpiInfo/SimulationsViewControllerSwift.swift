...
class SimulationsViewControllerSwift: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    if UIDevice.currentDevice().userInterfaceIdiom == .Pad {
      ...
      v2 = UIView(frame: CGRectMake(self.view.frame.size.width / 2.0 - 30.0, CGFloat(sqrtf(800)) + 60.0, 60, 90))
      v2.backgroundColor = UIColor.clearColor()
      self.view.addSubview(v2)
      let casecontrolButton = UIButton(frame: CGRectMake(0, 0, 60, 60))
      casecontrolButton.layer.cornerRadius = 10.0
      casecontrolButton.layer.masksToBounds = true
      casecontrolButton.addTarget(self, action: "casecontrolButtonTouched", forControlEvents: .TouchUpInside)
      if UIScreen.mainScreen().scale > 1.0 {
        casecontrolButton.setImage(UIImage(named: "SimulateCaseControl.png"), forState: .Normal)
      } else {
        casecontrolButton.setImage(UIImage(named: "SimulateCaseControlNR.png"), forState: .Normal)
      }
      v2.addSubview(casecontrolButton)
      let label3 = UILabel(frame: CGRectMake(-8, 64, 76, 12))
      let label4 = UILabel(frame: CGRectMake(0, 76, 60, 14))
...
//            imageData.writeToFile("/Users/zfj4/CodePlex/temp/Simulations5.png", atomically: true)
  ...
  func casecontrolButtonTouched() {
    ...
    self.casecontrolSimulationView.changeFrame(CGRectMake(v2.frame.origin.x, v2.frame.origin.y, 60, 60))
    ...
    UIView.animateWithDuration(0.2, delay: 0.0, options: UIViewAnimationOptions.CurveLinear, animations: {
      self.casecontrolSimulationView.changeFrame(self.fadingColorView.frame)
        ...
        (value: Bool) in
        self.customBackButton.removeTarget(self, action: "popCurrentViewController", forControlEvents: .TouchUpInside)
        self.customBackButton.addTarget(self, action: "removeCasecontrolView", forControlEvents: .TouchUpInside)
    })
  }
  ...
  func removeCasecontrolView() {
    UIView.animateWithDuration(0.2, delay: 0.0, options: UIViewAnimationOptions.CurveLinear, animations: {
      self.casecontrolSimulationView.changeFrame(CGRectMake(self.v2.frame.origin.x, self.v2.frame.origin.y, 60, 60))
      }, completion: {
        (value: Bool) in
        self.casecontrolSimulationView.killThread = true
        self.casecontrolSimulationView.removeFromSuperview()
        self.customBackButton.removeTarget(self, action: "removeCasecontrolView", forControlEvents: .TouchUpInside)
        self.customBackButton.addTarget(self, action: "popCurrentViewController", forControlEvents: .TouchUpInside)
    })
  }
